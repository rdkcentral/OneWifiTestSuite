...
int test_step_param_dml_reset::step_execute()
{
    ...
    if (webconfig_decode(&webconfig_data->webconfig, &dml_data, json_data) == webconfig_error_none){
    ...
    for (count = 0; count < ARRAY_SIZE(subdoc_array); count++) {
        ...
        if (ret != webconfig_error_none) {
            wlan_emu_print(wlan_emu_log_level_err, "%s:%d: Encode failed for Test Step Num : %d for subdoc : %d\n",
                    __func__, __LINE__, step->step_number, subdoc_array[count]);
            step->test_state = wlan_emu_tests_state_cmd_abort;
            return RETURN_ERR;
        }
        ...
        if (ret != RETURN_OK) {
            wlan_emu_print(wlan_emu_log_level_err, "%s:%d: rbus send failed for Test Step Num : %d for subdoc : %d\n",
                    __func__, __LINE__, step->step_number, subdoc_array[count]);
            step->test_state = wlan_emu_tests_state_cmd_abort;
            free(json_data);
            return RETURN_ERR;
        ...
        wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d: subdoc : %d\n",
                __func__, __LINE__, subdoc_array[count]);
        free(json_data);
    }
    ...
    if (count == ARRAY_SIZE(subdoc_array)) {
        ...
        step->test_state = wlan_emu_tests_state_cmd_abort;
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: Abort called for dml reset, execute update for %d from %d\n",
                __func__, __LINE__, count, ARRAY_SIZE(subdoc_array));
    }
